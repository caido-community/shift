...
import {
  ...
  removeLearnings,
  updateLearning,
} from "@/stores/learnings/store.effects";
...
const { sdk, dispatch } = learningsStore;

const learnings = computed(() => learningsStore.entries);
...
const toggleRowSelection = (row: LearningRow) => {
  if (isRowSelected(row)) {
    selectedRows.value = selectedRows.value.filter((selected) => selected.index !== row.index);
  } else {
...
const commitLearning = async (index: number) => {
  ...
  try {
    await updateLearning(sdk, dispatch, { index, content: draft });
  } finally {
...
const handleAddLearning = async () => {
  ...
  try {
    await addLearning(sdk, dispatch, { content: value });
    newLearning.value = "";
  } finally {
...
const handleDeleteRow = async (index: number) => {
  await removeLearnings(sdk, dispatch, { indexes: [index] });
  resetSelection();
};
...
const handleBulkDelete = async () => {
  ...
  await removeLearnings(sdk, dispatch, { indexes });
  resetSelection();
};
...
const handleClearAll = async () => {
  ...
  await clearLearnings(sdk, dispatch);
  resetSelection();
};
</script>
    ...
    <Card
      ...
      <template #content>
        <div class="flex items-center justify-between">
          <div>
            <h2 class="text-base font-bold">Learnings</h2>
            <p class="text-sm text-surface-400">
              Manage project-scoped learnings to share durable insights, IDs, and notes with your
              agents.
            </p>
          </div>
        ...
        <div class="flex flex-col h-full overflow-hidden">
          <div class="flex items-center justify-between gap-2 p-4 pb-0">
            <div class="text-sm text-surface-400">
              {{ rows.length }} learning{{ rows.length === 1 ? "" : "s" }} stored for this project.
            </div>
          </div>
          ...
          <div class="flex-1 overflow-hidden p-4">
            <div
              ...
              <div class="flex flex-col gap-1">
                <h3 class="text-base font-semibold text-surface-200">No learnings yet</h3>
                <p class="text-sm text-surface-400">
                  Capture your first learning below to help the agents remember important context.
                </p>
              </div>
            </div>
            ...
            <DataTable
              ...
              :pt:table="{ class: 'h-full w-full align-top' }">
              <Column class="w-12">
                <template #header>
                  <Checkbox
                    binary
                ...
                <template #body="{ data }">
                  <div class="flex flex-col gap-1">
                    ...
                    <span class="text-xs text-surface-500">
                      Press Ctrl+Enter (or Cmd+Enter) to save without leaving the field.
                    </span>
                  </div>
                </template>
              </Column>
